...
An optional **paid OpenAI-compatible provider** is tried **first**. It needs both `OPENAI_API_KEY` and `OPENAI_MODEL` — a lone key is skipped with a log warning, and no model is assumed. It speaks Chat Completions (`POST {OPENAI_BASE_URL}/chat/completions` with Bearer auth), so the base URL can point at OpenAI or any compatible gateway. Reasoning models reject a non-default `temperature`, so this path omits `temperature` and caps output with `max_completion_tokens` (`OPENAI_MAX_TOKENS_FIELD` overrides; `omit` drops the cap), and `OPENAI_REASONING_EFFORT` is sent only when set. Token usage reported by the provider is logged so paid spend is auditable.
