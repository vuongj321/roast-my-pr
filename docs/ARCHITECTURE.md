...
| Paid gateway refuses `reasoning_effort`, the token cap, or `temperature` | The field is shed and the call retried with a plainer body (last attempt: `model` + `messages`); the roast still comes from the paid provider |
